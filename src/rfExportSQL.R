...
BuildTermSumSQLExpression <- function(last.term, n.terms, max.sqle.length, term.prefix = "t")
{
  # Returns a string of the form "t_last.term + t_(last.term + 1) +...+ t_n.terms"
  # truncated so as not to exceed max.sqle.length in length.
  stopifnot(n.terms >= last.term)
  out.str <- paste(term.prefix, last.term:n.terms, sep="", collapse="+")
  if (nchar(out.str) > max.sqle.length) {
    out.str <- substr(out.str, 1, max.sqle.length)
...
BuildRulesOnlyScoringClause <- function(db.type, n.terms, max.sqle.length)
{
  # The SQL expression generated by BuildScoringClause(), c0 + c1*r1(x) + ..., can
  # result in a "expression too complex" error for some SQL variants. In this case,
  ...
  #    max.sqle.length : max sql expression length (in number of characters)
  # Returns: 
  ...
  last.term <- 0
  repeat {
    sum.str <- BuildTermSumSQLExpression(last.term, n.terms, max.sqle.length)
  ...
  score.sum.sql <- BuildTermSumSQLExpression(1, length(partial.sum.strs), max.sqle.length=255, term.prefix="s")
...
ExportModel2SQL <- function(model.path, out.path = model.path, levels.fname = "xtrain_levels.txt", out.fname = "rules_forSQL.txt", 
                            merge.dups = FALSE, expand.low.count.levels = FALSE,
                            export.type = "score", db.type = "SQLServer", max.sqle.length = 500) 
{
  ...
  terms.df <- ConvertTermsToSQL(terms, db.type, x.levels.fname = file.path(model.path, kMod.x.levels.fname),
      x.levels.lowcount.fname = ifelse(expand.low.count.levels, file.path(model.path, kMod.x.levels.lowcount.fname), ""),
  ...
  # Write out SQL clause
  sink(file.path(out.path, out.fname))
  cat(sql.str, "\n")
  sink()

  if (export.type == "rulescoeff") {
    out.fname2 <- paste0(out.fname, "2_part1")
    score.str <- BuildRulesOnlyScoringClause(db.type, n.terms = nrow(terms.df), max.sqle.length)
    sink(file.path(out.path, out.fname2))
    cat("\n", score.str$score.sum.sql, "\n")
    sink()
    out.fname2 <- paste0(out.fname, "2_part2")
    sink(file.path(out.path, out.fname2))
    cat("\n", score.str$partial.sum.sql, "\n")
    sink()
  }

  out.fname_lcl = paste0(out.fname, "lowcount")
  ...
  for(i in 1:length(tmp)){
      var = tmp[[i]]$var
      l = tmp[[i]]$levels
      valueList = paste(paste("'",l[1:length(l)-1], "'",sep=""), collapse = ",")
      if(grepl("_LowCountLevels_", tmp[[i]])[2]){
          if (db.type == "Netezza" || db.type == "SQLServer" || db.type == "HiveQL") {
              tmpStr = sprintf("case when %s in (%s) then cast(%s as varchar(25)) else '_LowCountLevels_' end as %s", var, valueList, var, var)
